...
// it panics if the buffer is not large enough
func (t WithdrawCall) EncodeTo(buf []byte) (int, error) {
	dynamicOffset := WithdrawCallStaticSize // Start dynamic data after static section

	// Wad (static)

	if err := abi.EncodeBigInt(t.Wad, buf[0:32], false); err != nil {
		return 0, err
	}
...
func (t WithdrawCall) Encode() ([]byte, error) {
	buf := make([]byte, t.EncodedSize())
	if _, err := t.EncodeTo(buf); err != nil {
		return nil, err
	}
...
func (t *WithdrawCall) Decode(data0 []byte) error {
	if len(data0) < WithdrawCallStaticSize {
		return fmt.Errorf("insufficient data for WithdrawCall")
	}

	// t.Wad (static)
	t.Wad = new(big.Int).SetBytes(data0[0:32])
...
func (e DepositEventIndexed) EncodeTopics() []common.Hash {
	...
	// Encode indexed field Dst
	{
		var buf common.Hash

		// Dst (static)
		copy(buf[0+12:0+32], e.Dst[:])
...
func (e *DepositEventIndexed) DecodeTopics(topics []common.Hash) error {
	...
	// Dst (static)
	{
		...
		// e.Dst (static)
		copy(e.Dst[:], data[offset+12:offset+32])
...
// WithdrawalEvent represents an ABI event
...
func (e WithdrawalEventIndexed) EncodeTopics() []common.Hash {
	topics := make([]common.Hash, 0, 2)
	topics = append(topics, WithdrawalEventTopic)

	// Encode indexed field Src
	{
		var buf common.Hash

		// Src (static)
		copy(buf[0+12:0+32], e.Src[:])
...
// DecodeTopics decodes indexed fields of Withdrawal event from topics
func (e *WithdrawalEventIndexed) DecodeTopics(topics []common.Hash) error {
	if len(topics) < 2 {
		return fmt.Errorf("insufficient topics for Withdrawal event")
	}

	// Check event signature
	if topics[0] != WithdrawalEventTopic {
		return fmt.Errorf("invalid event signature for Withdrawal event")
	}
	...
	// Src (static)
	{
		...
		// e.Src (static)
		copy(e.Src[:], data[offset+12:offset+32])
...
// it panics if the buffer is not large enough
func (t WithdrawalEventData) EncodeTo(buf []byte) (int, error) {
...
func (t WithdrawalEventData) Encode() ([]byte, error) {
	buf := make([]byte, t.EncodedSize())
	if _, err := t.EncodeTo(buf); err != nil {
		return nil, err
	}
...
func (t *WithdrawalEventData) Decode(data0 []byte) error {
	if len(data0) < WithdrawalEventDataStaticSize {
		return fmt.Errorf("insufficient data for WithdrawalEventData")
	}
